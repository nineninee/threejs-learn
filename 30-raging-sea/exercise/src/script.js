...
const waterMaterial = new THREE.ShaderMaterial({
    uniforms: {
        ...
        uTime: { value: 0 },
        uBigWavesSpeed: { value: new THREE.Vector2(0.75) },
...
gui.add(waterMaterial.uniforms.uBigWavesFrequency.value, 'y')
    .min(0).max(10).step(0.001).name('uBigWavesFrequencyZ')

gui.add(waterMaterial.uniforms.uBigWavesSpeed.value, 'x')
    .min(0).max(4).step(0.001).name('uBigWavesSpeedX')
gui.add(waterMaterial.uniforms.uBigWavesSpeed.value, 'y')
    .min(0).max(4).step(0.001).name('uBigWavesSpeedZ')

gui.addColor(debugObject, 'depthColor').onChange(() => {
    waterMaterial.uniforms.uDepthColor.set(debugObject.depthColor)
})
gui.addColor(debugObject, 'surfaceColor').onChange(() => {
    waterMaterial.uniforms.uSurfaceColor.set(debugObject.depthColor)
})
...
gui.add(waterMaterial.uniforms.uColorMultiplier, 'value')
    .min(0).max(4).step(0.001).name('uColorMultiplier')
